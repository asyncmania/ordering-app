...
export class CategoryList {
  ...
  getContent(): HTMLElement {
    return (
      <div> { ["All", ...this.props.categories].map(c => this.getCategoryButton(c))} </div>
    )
  }
  ...
  getCategoryButton(cat?: string): HTMLElement { 
    ...
    return <button className={ `btn btn-block ${btnClass}` } onclick={ () => this.props.callback(cat)}> { cat } </button> 
  }
